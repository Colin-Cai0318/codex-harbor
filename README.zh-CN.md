...
默认关闭的「周额度重置后发送 Luna 消息」开关位于控制台任务池栏。
开启后，Harbor 记录周额度重置时间；到达该时间且新查询的额度可用时，
用 `gpt-5.6-luna / low` 在独立、只读的对话中发送一条要求回复 `OK` 的消息。
它独立于任务池暂停/冻结，需要 Harbor 运行并联网；不使用 Luna Reserve。
关闭开关会停止后续发送，已发出的请求无法撤回。

SQLite 记录保证每个已记录的重置时间最多尝试一次，可跨重启去重。
超时或中断可能意味着请求已送达，因此不会自动重发。控制台显示结果以及
后续观察到的下次重置时间；发送成功不等于已证明新一周计时开始，最终以账户数据为准。
`GET /api/weekly-ping` 查看状态；`PATCH /api/weekly-ping` 配合
JSON `{"enabled": true}` 开启（`false` 关闭）。
